...
def volume_per_brain_region(
    ...
    if save_table:
        if output_table:
            ...
            output_table = os.path.join(os.getcwd(), "volume_for_each_label.csv")
        ...
        for ilabel, label in enumerate(unique_labels):
            if volumes[ilabel]:
                if len(label_names) == len(unique_labels):
                    if verbose:
                        print(
                            f"{label_names[ilabel]} ({label}) volume = {volumes[ilabel]:2.3f}mm^3\n"
                        )
                    fid.write(
                        f"{label_names[ilabel]}, {label}, {volumes[ilabel]:2.3f}\n"
                    )
                else:
                    if verbose:
...
def thickinthehead(
    ...
    if save_table:
        if output_table:
            ...
            output_table = os.path.join(
                os.getcwd(), "thickinthehead_for_each_label.csv"
            )
        fid = builtins.open(output_table, "w", encoding="utf-8")
        if names:
    ...
    voxarea = (
        voxsize[0] * voxsize[1] + voxsize[0] * voxsize[2] + voxsize[1] * voxsize[2]
    ) / 3
    ...
    for ilabel, label in enumerate(labels):
        ...
        label_inner_edge_area = voxarea * len(np.where(inner_edge_data == label)[0])
        if label_inner_edge_area:
            if use_outer_edge:
                label_outer_edge_area = voxarea * len(
                    np.where(outer_edge_data == label)[0]
                )
                label_area = (label_inner_edge_area + label_outer_edge_area) / 2.0
            else:
            ...
            if save_table:
                if names:
                    if verbose:
                        print(
                            f"{name} ({label}) thickinthehead thickness = "
                            f"{thickness:2.2f}mm"
                        )
