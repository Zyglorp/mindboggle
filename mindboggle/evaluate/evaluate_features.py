...
def evaluate_deep_features(
    ...
    if sulci_file:
        ...
        sulcus_indices = [i for i, x in enumerate(sulci) if x not in excludeIDs]
        segmentIDs = sulci
    ...
    border_indices, border_label_tuples, unique_border_label_tuples = extract_borders(
        sulcus_indices,
        labels,
        neighbor_lists,
        ignore_values=[],
    ...
    for isulcus, label_pairs in enumerate(dkt.sulcus_label_pair_lists):
        ...
        label_pair_border_indices = [
            x
            for i, x in enumerate(border_indices)
    ...
    if len(np.unique(label_borders)) > 1:
        ...
        for ifeature in range(nfeatures):
            feature_distances = [x for x in distance_matrix[:, ifeature] if x != -1]
            feature_to_border_mean_distances[ifeature] = np.mean(feature_distances)
            feature_to_border_sd_distances[ifeature] = np.std(feature_distances)
        ...
        if output_vtk_name:
            feature_to_border_distances_vtk = os.path.join(
                os.getcwd(), output_vtk_name + "_feature_to_border_mean_distances.vtk"
            )
            print(
                f"Write feature-to-border distances to {feature_to_border_distances_vtk}..."
            )
            write_vtk(
                feature_to_border_distances_vtk,
                points,
        ...
        nfeatures = min(np.shape(distance_matrix)[1], nsulcus_lists)
        for ifeature in range(nfeatures):
            border_distances = [x for x in distance_matrix[:, ifeature] if x != -1]
            border_to_feature_mean_distances[ifeature] = np.mean(border_distances)
            border_to_feature_sd_distances[ifeature] = np.std(border_distances)
        ...
        if output_vtk_name:
            border_to_feature_distances_vtk = os.path.join(
                os.getcwd(), output_vtk_name + "_border_to_feature_mean_distances.vtk"
            )
            print(
                f"Write border-to-feature distances to {border_to_feature_distances_vtk}..."
            )
            write_vtk(
                border_to_feature_distances_vtk,
                points,
...
if __name__ == "__main__":
    ...
    if measure_feature_distances:
            ...
            for n in range(1, number + 1):
                ...
                for isurf, surf in enumerate(surfs):
                    ...
                    labels_file = os.path.join(
                        ldir, "labels", surf, "relabeled_labels.DKT31.manual.vtk"
                    )
                    # --------------------------------------------------------
                    ...
                    if feature_type == "fundi":
                        if nmethod == 0:
                            features_file = os.path.join(
                                mdir, "features", surf, "fundus_per_sulcus.vtk"
                            )
                        else:
                            features_file = os.path.join(
                                fmethod_dir,
        ...
        # --------------------------------------------------------------------
        np.savetxt(
            fmethod + "_mean_distances_to_border_left.csv",
            feature_to_border_mean_distances_left,
        )
        np.savetxt(
            fmethod + "_sd_distances_to_border_left.csv",
            feature_to_border_sd_distances_left,
        )
        np.savetxt(
            fmethod + "_mean_distances_from_border_left.csv",
            border_to_feature_mean_distances_left,
        )
        np.savetxt(
            fmethod + "_sd_distances_from_border_left.csv",
            border_to_feature_sd_distances_left,
        )

        np.savetxt(
            fmethod + "_mean_distances_to_border_right.csv",
            feature_to_border_mean_distances_right,
        )
        np.savetxt(
            fmethod + "_sd_distances_to_border_right.csv",
            feature_to_border_sd_distances_right,
        )
        np.savetxt(
            fmethod + "_mean_distances_from_border_right.csv",
            border_to_feature_mean_distances_right,
        )
        np.savetxt(
            fmethod + "_sd_distances_from_border_right.csv",
    ...
    if plot_and_describe_fundus_distances:
        ...
        for imethod, fmethod in enumerate(fmethods):
            ...
            desc5 = (
                "Standard deviation of distances from left "
                + fmethod_name
                + " to label borders"
            )
            desc6 = (
                "Standard deviation of distances from left label borders to "
                + fmethod_name
            )
            desc7 = (
                "Standard deviation of distances from right "
                + fmethod_name
                + " to label borders"
            )
            desc8 = (
                "Standard deviation of distances from right label borders to "
                + fmethod_name
            )
            ...
            feature_tables = [
                os.path.join(feature_tables_dir, x + ".csv") for x in tables
            ]
            ...
            for itable, feature_table in enumerate(feature_tables):
                ...
                data = pd.read_csv(data_file, sep=" ", index_col=False, header=None)
                if exclude_sulci:
                    data = data.iloc[
                        :,
                        [
                            x
                            for x in range(len(labels) + len(exclude_sulci))
                ...
                for ilabel, label in enumerate(labels):
                    for isubject, subject in enumerate(subjects):
                        ...
                        if np.isnan(value):
                            ...
                            rgb = [
                                np.int(255 * x)
                                for x in colors[np.int(255 * value / maxd)]
                            ]
    ...
    # ------------------------------------------------------------------------
    if compute_results:
        fmethods = ["mindboggle_fundi", "ForrestBao_fundi", "GangLi_fundi"]
        #'OlivierCoulon_fundi']
        ...
        for itable_list, table_list in enumerate(table_lists):
            ...
            for itable, table in enumerate(table_list):
                summary_file = table + "_summary.csv"
                data = pd.read_csv(summary_file, sep=",", index_col="Unnamed: 0")
        ...
        # --------------------------------------------------------------------
        np.savetxt(
            "sort_8tables_3methods-mb-bao-li.csv",
            sort_values.sum(axis=1),
            delimiter=",",
        )
        ...
        # --------------------------------------------------------------------
        np.savetxt("means_8tables_3methods-mb-bao-li.csv", mean_values, delimiter=",")
        # Save results as separate tables:
        np.savetxt(
            "mean-to-border_mb-bao-li.csv",
            (mean_values[0] + mean_values[2]) / 2,
            delimiter=",",
        )
        np.savetxt(
            "mean-from-border_mb-bao-li.csv",
            (mean_values[1] + mean_values[3]) / 2,
            delimiter=",",
        )
        np.savetxt(
            "sd-to-border_mb-bao-li.csv",
            (mean_values[4] + mean_values[6]) / 2,
            delimiter=",",
        )
        np.savetxt(
            "sd-from-border_mb-bao-li.csv",
            (mean_values[5] + mean_values[7]) / 2,
            delimiter=",",
        )
        ...
        # --------------------------------------------------------------------
        np.savetxt(
            "mean-of-max_8tables_3methods-mb-bao-li.csv", mean_max_values, delimiter=","
        )
        # Save results as separate tables:
        np.savetxt(
            "mean-of-max-to-border_mb-bao-li.csv",
            (mean_max_values[0] + mean_max_values[2]) / 2,
            delimiter=",",
        )
        np.savetxt(
            "mean-of-max-from-border_mb-bao-li.csv",
            (mean_max_values[1] + mean_max_values[3]) / 2,
            delimiter=",",
        )
        np.savetxt(
            "sd-mean-of-max-to-border_mb-bao-li.csv",
            (mean_max_values[4] + mean_max_values[6]) / 2,
            delimiter=",",
        )
        np.savetxt(
            "sd-mean-of-max-from-border_mb-bao-li.csv",
            (mean_max_values[5] + mean_max_values[7]) / 2,
            delimiter=",",
        )
        ...
        for itable, table in enumerate(min_values):
            for imethod in range(len(fmethods)):
                number_of_min_values[itable, imethod] = np.size(
                    np.where(table == imethod)
                )
        ...
        # --------------------------------------------------------------------
        np.savetxt(
            "number_of_min_values_8tables_3methods-mb-bao-li.csv",
