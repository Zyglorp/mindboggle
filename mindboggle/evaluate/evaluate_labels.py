...
def evaluate_volume_overlaps(labels, file1, file2, output_file="", save_output=True):
    """
...
def evaluate_surface_overlaps(
    labels, index, table1, table2, output_file="", save_output=True
):
    """
...
def evaluate_surface_overlaps_cpp(command, labels_file1, labels_file2, output_file):
    """
    ...
    if not output_file:
        output_file = (
            os.path.basename(labels_file1)
            + "_and_"
            + os.path.basename(labels_file2)
            + ".txt"
        )
...
if __name__ == "__main__":
    ...
    if plot_volume_overlaps:
        ...
        if dirnum == 1:
            vol_overlap_dir = os.path.join(
                vol_overlap_path, "volume_overlap_mindboggle_vs_ants_filled_labels"
            )
            file_append = "_ants_filled_labels_overlap.csv"
            title = (
                "Volume overlap: Manual labels vs. Mindboggle with ANTs-filled labels"
            )
        elif dirnum == 2:
            vol_overlap_dir = os.path.join(
                vol_overlap_path,
    ...
    if measure_surface_overlaps:
        ...
        for iname, name in enumerate(names):
            ...
            for n in range(1, number + 1):
                ...
                for surf in surfs:
                    file1 = os.path.join(
                        mindboggled, subject, "tables", surf, "vertices.csv"
                    )
                    file2 = os.path.join(
                        labels_dir, subject, "tables", surf, "vertices.csv"
                    )
                    print(file1)
                    print(file2)
                    output_file = f"{subject}_{surf}_surface_label_overlaps.csv"
                    evaluate_surface_overlaps(
                        dkt.cerebrum_cortex_DKT31_numbers,
                        index,
                        file1,
                        file2,
                        output_file,
    ...
    if measure_volume_overlaps:
        for iname, name in enumerate(names):
            ...
            for n in range(1, number + 1):
                ...
                evaluate_volume_overlaps(dkt.label_numbers, file1, file2, output_file)
    ...
    if plot_volume_overlaps:
        ...
        jaccard_file = os.path.join(
            vol_overlap_path, "subjects_by_labels_jaccards" + file_append
        )
        dice_file = os.path.join(
            vol_overlap_path, "subjects_by_labels_dices" + file_append
        )
        ...
        for ilabel, label in enumerate(labels):
            for isubject, subject in enumerate(subjects):
                ...
                rgb = [
                    np.int(255 * x)
                    for x in colors[np.int(255 * data.iloc[isubject, ilabel])]
                ]
