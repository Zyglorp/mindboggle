...
def read_vertices(filename)->list[int]:
    """
...
def read_lines(filename)->tuple[list[tuple[int,int]],list[float]]:
    """
...
def read_points(filename)->list[list[float]]:
    """
...
def read_faces_points(filename)->tuple[list[list[int]],list[list[float]],int]:
    """
...
def read_scalars(filename, return_first=True, return_array=False)->tuple[list[list[int|float]],list[str]]:
    """
...
def read_vtk(input_vtk, return_first=True, return_array=False)-> tuple[list[tuple[float,float,float]],list[int],list[tuple[int,int]],list[list[int]],list[any]|list[list[float|int]],str|list[str],int,str]:
    """
...
def write_points(Fp, points, dataType="float")->None:
    """
...
def write_vertices(Fp, indices)->None:
    """
...
def write_scalars(Fp, scalars, scalar_name, begin_scalars=True, scalar_type="float")->None:
    """
...
import numpy as np #TODO: MOVE TO TOP
def rewrite_scalars(
    input_vtk:str,
    output_vtk:str,
    new_scalars:list[list[float]]|list[float]|np.ndarray[float],
    new_scalar_names:str|list[str]=["scalars"],
    filter_scalars:list[any] | np.ndarray[any] |None=[],
...
def explode_scalars(
    input_indices_vtk:str,
    input_values_vtk:str="",
    output_stem:str="",
    exclude_values:list[any]=[-1],#TODO: find out what array in this case means
    background_value:int|float=-1,
    output_scalar_name="scalars",
    remove_background_faces:bool=True,
    reindex:bool=True,
    verbose:bool=False,
):
    """
