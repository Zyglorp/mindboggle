...
if __name__ == "__main__":
    description = (
        "calculate colormap for labeled image;"
    ...
    parser.add_argument(
        "output_dirname", help="path to the folder storing temporary files and result"
    )
    ...
    if not os.path.isfile(matrix_filename) or not os.path.isfile(labels_filename):
        labels, matrix = label_adjacency_matrix(
            args.label_filename, out_dir=args.output_dirname
        )[:2]
    ...
    if not os.path.isfile(colors_filename):
        label_colors = colors.group_colors(
            colormap,
            args.label_filename,
            IDs=labels,
